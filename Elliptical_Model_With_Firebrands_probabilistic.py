...
## Physical Paremeters:
w = 27                                          # windspeed in km/h
r_m = 2.5                                       #corresponds to 27km/h windspeed; see p.48 "User Guide to the Canadian Forest Fire Behaviour Prediction System"
...
forx = 1500                      # Length of forest in meters
fory = 2500                      # Width of forest in meters
river_width = 1100                # Width of river in metres
...
y_0s = [40]                                # y-co-ordinates for our initial condition(s)
...
fb_mass_cutoff = 0.075

## Iteration Parameters
timestep_length = 5                                                    #the time elapsed between each timestep
minutes = 4*60                                                            #number of minutes we want to run simulation for
...
ffmc = 96       # Fine Fuel Mositure Content
dmc = 45        # Duff moisture content
dc = 367        # Drought moisture content
fmc = 85        # foliar moisture content
w = 27
r_m = 2.5
## Misc. Parameters
prob_thresh = 0.50 # The lower bound on the probability required to start a fire
...
def forest_grid_init():
    ...
    for i in range(forest_x):
        ...
        for j in range(forest_y):
            if abs(j - river_centre) <= river_w/2:
...
def fire_line_intensity(dmc, dc, w, ffmc):
    ...
    def rate_of_spread(a, b, c, w, ffmc):
    ...
    def crown_fraction_burned():
        if critical_rate_spread() < rate_of_spread(a, b, c, w, ffmc):
            cfb = 1 - np.exp(-0.23*(rate_of_spread(a, b, c, w, ffmc)-critical_rate_spread()))
        else:
            cfb = 0
        return cfb
    ...
    def fire_intensity():
        I = round(300 * total_fuel_consumed() * rate_of_spread(a, b, c, w, ffmc))
        return I

    def p_ign():
        p = 1 / (1 + np.exp(-(b0 + b1 * ffmc + b2 * dmc + b3 * initial_spread_index(ffmc, w))))
        return p
    return [fire_intensity(), rate_of_spread(a, b, c, w, ffmc), p_ign()]
...
def spotting(dmc,dc, w, ffmc, fb_mass_cutoff):
    ...
    def loftedheight():
        ...
        zmax = gamma * ((Pa / Pb) * (Cd / r0)) ** (3 / 2) * fire_line_intensity(dmc, dc, w, ffmc)[0] ** (5 / 3)
    ...
    def launching():
        ...
        x0 = 0.7 * Z * (bouy * fire_line_intensity(dmc, dc,w, ffmc)[0] / (w ** 3)) ** (1 / 2)
        ...
        def P_ign():
            p_no_fb = fire_line_intensity(dmc, dc,w, ffmc)[2]
            # Probability of ignition in percent
            if fb_mass <= fb_mass_cutoff:      ####This was changed from the original model
                p = 0
            else:
                p = p_no_fb
            return float(p)
...
def Max_Fire_Length(timestep, j_0, dmc, dc, delta, forest_y, w, ffmc):                           #finds the hypothetical max length, given the timestep, assuming no obstacles (this returns 2*y-axis of the ellipse)
    M = 0
    j = j_0
    while timestep*fire_line_intensity(dmc,dc, w, ffmc)[1] > abs(j - j_0)*delta:
        M = M + 1
        j = j + 1
...
def EllipticalSpread(timestep, times, x_0s, y_0s, r_m, dmc, dc, delta, forest_x, forest_y, grid, timestep_length, burn_time, fire_brand_rate, fire_brand_time, w, ffmc, num_fb, fb_mass_cutoff, prob_fb): #our method of updating the fire growth. The explanation of each parameter is the same as above where we define all the parameters
    ...
    for p in range(0, len(times)):
        M_ts.append((Max_Fire_Length(timestep - times[p], y_0s[p], dmc, dc, delta, forest_y, w, ffmc))/2)
    ...
    # Now, we launch a certain number of firebrands per burning cell, specified by the timestep length and the rate of fire brands per cell              
                ...
                for k in range(0, int(timestep_length*fire_brand_rate*Fire_Brand_Update_Time_length(timestep_length, fire_brand_rate))):
                    L = spotting(dmc, dc, w,ffmc,fb_mass_cutoff)
                    if j + L[0] < forest_y:
                        if grid[i][j + L[0]] == fuel and j + L[0] < river_centre + river_w/2 and L[1] > 0.5:
                            grid[i][j + L[0]] = -burn_time + (-fire_brand_time)
                        if j + L[0] >= river_centre + river_w/2: ##check if the firebrand crossed the river
                            num_fb = num_fb + 1                 ##keep track of the number of firebrands that cross
                            prob_fb.append(L[1])                ##Keep track of its probability of starting a fire
    ...
    return [grid, times, x_0s, y_0s, num_fb]



init_grid = forest_grid_init() #Initialize the grid
grid = init_grid
...
for k in range(30,70,5):
    ...
    for k in range(0,sample_size):
        ...
        for t in timesteps:
            if t > 0:
                [grid, times, x_0s, y_0s, num_fb] = EllipticalSpread(t, times, x_0s, y_0s, r_m, dmc, dc, delta, forest_x, forest_y, grid, timestep_length, burn_time, fire_brand_rate, fire_brand_time,w,ffmc, num_fb, fb_mass_cutoff,prob_fb)
        if num_fb != 0:
            for q in prob_fb:
                p = (1-q)*p
        prob_fb_data.append(1 - p)
        print('probability of Ignition Across River in 4 hours = ' + str(1 - p))
    print('average probability over sample size = ' + str(sample_size) + ' is ' + str(sum(prob_fb_data)/sample_size))
...
#plt.bar(y_pos, time_data_freq, align='center', alpha=0.5)
#plt.xticks(y_pos, tuple(Ts))
#plt.ylabel('Frequency')
#plt.title('time (minutes) to cross river')
...
#        [grid, times, x_0s, y_0s, over_river, D] = EllipticalSpread(t, times, x_0s, y_0s, r_m, dmc, dc, delta, forest_x, forest_y, grid, timestep_length, burn_time, fire_brand_rate, fire_brand_time,over_river)

        
#grid_animation(grid_list)


#grid_animation([forest_grid_init()])
